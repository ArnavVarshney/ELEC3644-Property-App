    ...
    var body: some View {
        NavigationStack{
            VStack(alignment: .leading){
                TextField("", text: $note, axis: .vertical).font(.system(size: 15)).focused($foc, equals: true)
                Text("\(note.replacingOccurrences(of: " ", with: "").count)/\(TEXT_LIMIT) characters").font(.footnote).foregroundStyle(.neutral60).padding(.init(top: 10, leading: 0, bottom: 5, trailing: 0))
            }
            ...
            HStack{
                ...
                Button{
                    ...
                    Text("Save").bold().padding(.init(top: 15, leading: 20, bottom: 15, trailing: 20))
                }
