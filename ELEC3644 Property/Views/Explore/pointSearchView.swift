...
enum Area: String, CaseIterable {
    ...
    var districts: [District] {
        ...
        case .newTerritories:
            return [.kwaiTsing, .tsuenWan, .tuenMun, .yuenLong, .north, .taiPo, .shaTin, .saiKung, .islands]
        }
    }
}
        ...
        NavigationStack{
            VStack{
                Form {
                    Section(header: Text("Select Area")) {
                        ...
                        .onChange(of: selectedArea) { oldValue, newValue in
                            ...
                            if !newValue.districts.contains(selectedDistrict ?? District.centralAndWestern) {
                ...
                if let mapItem = mapItem{
                    NavigationLink(destination: EnlargeMapView_V2(currentMenu: $currentMenu, startMapCameraLocation: .customLocation(latitude: mapItem.placemark.coordinate.latitude, longitude: mapItem.placemark.coordinate.longitude)) // Pass any required data here
